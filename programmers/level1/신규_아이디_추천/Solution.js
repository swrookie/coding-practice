function solution(new_id) {
    ...
    new_id = new_id.replace(new RegExp(/[^a-z0-9-_//.]/gi), '');
    console.log(new_id);

    // 3단계 
    // 마침표(.)가 2번 이상 연속된 부분을 하나의 마침표(.)로 치환

    new_id = new_id.replace(new RegExp(/[/\\./]{2,}/gi), '.')
    console.log(new_id);
    ...
    if (new_id.startsWith('.') || new_id.endsWith('.')) {
        new_id = new_id.replace(new RegExp(/^[.]|[.]$/gi), '')
    }
    console.log(new_id);
    ...
    if (new_id.length <= 2) {
        var lastStr = new_id.substring(new_id.length - 1);
